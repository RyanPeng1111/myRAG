...
| 圖譜 | NetworkX，本機檔案 |
| Embedding | FastEmbed / ONNX Runtime，CPU、多語模型、384 維；也可設定公司 API |
| OCR | RapidOCR，CPU |
...
建議保留 **Python / FastAPI / LightRAG**，沿用公司的 **S3 / MinIO**，新增一套 **PostgreSQL + pgvector**。企業共用階段再部署到既有 K8s。這是未來演進方案，不是本週驗證 demo 的新增前置要求。

| 層次 | 建議 | 原因及必要工作 |
|---|---|---|
| 原檔、圖片、解析產物 | 既有 MinIO / S3 | 大型檔案放物件儲存；新增儲存轉接層，保留文件 ID、版本、頁碼及來源映射 |
...
| 模型 | 公司 LLM API；CPU Embedding 起步 | 有公司 Embedding API 再評估切換；無 GPU 不阻礙第一階段驗證 |
| 部署 | 公司 K8s、內部 registry、既有監控 | 先以小規模共用服務起步，再依測量結果增加副本 |

LightRAG 上游文件列出 PostgreSQL 的四類儲存介面；PGTableGraphStorage 不需 AGE。這些介面也出現在本專案鎖定版本的儲存註冊表中，但本專案尚未實測 PostgreSQL 整合。實作時須選定公司允許、上游支援的 PostgreSQL 版本，建立獨立整合測試，不能用上游支援取代本專案驗收。[LightRAG API Server 文件](https://github.com/HKUDS/LightRAG/blob/main/docs/LightRAG-API-Server.md)
...
你熟悉 Oracle，確實是維運優勢；但目前鎖定的 LightRAG 1.5.7 儲存註冊表沒有 Oracle 後端。選 Oracle 作為整個 RAG 儲存層，需要自行維護轉接、查詢語意與版本相容性，開發成本通常比沿用上游介面高。

Oracle 新版具有 AI Vector Search，但公司現有版本、可用功能及權限尚未確認，不能假設既有 Oracle 就能直接接上。[Oracle AI Vector Search 官方說明](https://docs.oracle.com/en/database/oracle/oracle-database/26/vecse/overview-ai-vector-search.html)

因此：若公司能提供 PostgreSQL，優先用它整合 RAG 儲存；若公司政策只允許 Oracle，再確認版本並評估轉接成本，不急著改寫引擎。只有確實需要整合既有業務資料時，才讓 Oracle 同時參與，避免只是為了熟悉而多維運一套資料庫。PostgreSQL 是否容易取得，仍是選型前需要確認的條件。
...
保留 Windows 本機架構，先在內網驗證研究報告、PPT、表格和圖片。建立一組由 RD 確認答案與來源的問題，涵蓋實驗尋找、比較、新人問答、影響分析與靈感探索。記錄搜尋命中、引用位置、解析缺漏、回答時間及 LLM 用量。不要只看知識圖譜是否漂亮。

### 2. 小範圍共用：先處理資料與身分

接 MinIO、PostgreSQL、背景工作與公司身分系統。雖然 demo 延後權限，正式分享給多人前仍應落實文件及知識庫存取控制：檢索候選、圖譜關係、原檔、圖片、下載與快取都要一致限制，不能只在生成回答後遮字。LightRAG workspace 也不能直接當作完整的企業 ACL。
...
6. K8s 常用 Linux 容器；目前的 Windows x64 離線 wheelhouse 不能直接搬進 Linux。未來以公司核准的基底、內部套件來源與模型建立 image，再經內部 registry／掃描流程發布；不要求現在的本機 demo 改用 Docker 或 WSL。

資料搬遷限制與上游儲存配置參考：[LightRAG 儲存文件](https://github.com/HKUDS/LightRAG/blob/main/docs/ProgramingWithCore.md)。以上連結指向上游現行文件，實作仍以本專案鎖定版本及公司環境驗證結果為準。
